...
Data Source:
- Health Expense data
- Available via Seaborn Github

Assumptions:
- The data contains columns like:
  age, country, health_expenditure USD, Life Expectancy

Terminal command to run this file from the root project folder:

uv run python -m datafun.app_bjean_healthexp
...
DATASET_NAME: Final[str] = "healthexp"
...
def descriptive_stats(df_clean: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    mean_Spending_USD = np.mean(df_clean["Spending_USD"])
    std_Spending_USD = np.std(df_clean["Spending_USD"])
    min_Spending_USD = np.min(df_clean["Spending_USD"])
    max_Spending_USD = np.max(df_clean["Spending_USD"])
    range_Spending_USD = np.ptp(df_clean["Spending_USD"])  # peak to peak (max - min)

    # Log the example results with formatting
    LOG.debug("Health Expenditure Statistics (using numpy):")
    LOG.debug(f"  Mean: {mean_Spending_USD:.2f} USD")
    LOG.debug(f"  Std Dev: {std_Spending_USD:.2f} USD")
    LOG.debug(f"  Min: {min_Spending_USD:.2f} USD")
    LOG.debug(f"  Max: {max_Spending_USD:.2f} USD")
    LOG.debug(f"  Range: {range_Spending_USD:.2f} USD")
...
def make_plots(df_clean: pd.DataFrame) -> None:
    ...
    LOG.info("------   Set y to Spending_USD ------------------")
    ...
    box_plt.set_title("Health Expenditure by Country")
    ...
    plt.title("Health Expenditure vs. Life Expectancy (with Linear Regression)")
...
def summarize(df: pd.DataFrame, df_clean: pd.DataFrame) -> None:
    ...
    LOG.info(
        "  Spending_USD and Life_Expectancy show a strong positive correlation (~0.8)"
    )

    LOG.info("Suggested next step: ")
    LOG.info("  Model Spending_USD ~ Life_Expectancy with linear regression")
